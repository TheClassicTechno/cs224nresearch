...
#SFT baseline for Qwen-7B using Hugging Face Transformers and LoRA
...
def main():
    ...
    parser = argparse.ArgumentParser(description="SFT baseline for Qwen-7B")
    ...
    model_name = get_cfg("model_name", "Qwen/Qwen2-7B")
    ...
    output_dir = get_cfg("output_dir", "sft_qwen7_ckpt")
    ...
    if use_lora and (str(use_lora).lower() == "true" or use_lora is True):
        ...
        lora_config = LoraConfig(
            r=lora_r,
            lora_alpha=lora_alpha,
            target_modules="all-linear",  # Qwen3.5-2B DeltaNet hybrid requires all-linear
            lora_dropout=lora_dropout,
            bias="none",
