"""
SFT training: Qwen/Qwen3.5-4B on the training split of mli5/medquad-sycophancy.

Trains with LoRA (rank 8, alpha 16, target_modules='all-linear') for 2 epochs
and pushes the merged checkpoint to HF Hub.

Run:
    modal run modal_training/4bmodelSFT.py
    modal run --detach modal_training/4bmodelSFT.py   # keep running if terminal disconnects
"""

import os
import modal

app = modal.App("qwen35-4b-sft-train")
...
HF_REPO_ID = "mli5/qwen3.5-4b-sft-medquad"
OUTPUT_DIR = "/tmp/sft_qwen3.5_4b_ckpt"
...
@app.function(
    image=image,
    gpu="A100",        # 40 GB VRAM — needed for Qwen3.5-4B with LoRA
    timeout=86400,     # 24h
...
def run_sft_train():
    ...
    import wandb
    wandb.init(project="qwen35-4b-sft", name="sft-medquad")
