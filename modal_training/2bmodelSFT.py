"""
SFT training: Qwen/Qwen3.5-2B on the training split of mli5/medquad-sycophancy.

Trains with LoRA (rank 8, alpha 16, target_modules='all-linear') for 2 epochs
and pushes the merged checkpoint to HF Hub.

Run:
    modal run modal_training/sft.py
    modal run --detach modal_training/sft.py   # keep running if terminal disconnects
"""

import os
import modal

app = modal.App("qwen35-sft-train")
...
HF_REPO_ID = "technojules/qwen3.5-2b-sft-medquad"
OUTPUT_DIR = "/tmp/sft_qwen3.5_ckpt"
...
@app.function(
    image=image,
    gpu="A10G",        # 24 GB VRAM — comfortably fits Qwen3.5-2B with LoRA
    timeout=86400,     # 24h
...
def run_sft_train():
    ...
    # ── 3. LoRA ──────────────────────────────────────────────────────────────
    # Qwen3.5-2B uses a DeltaNet hybrid architecture; "all-linear" covers all
    # linear projections including the SSM mixer weights.
